...
# Compare files
echo "================================================="
echo "Test-3: Comparing OptiX and Vulkan output files for Realworld Datasets..."

if diff "$file1" "$file2" > "$diff_file"; then
    echo "Test-3 PASS: The output files are identical."
    rm -f "$diff_file"
    exit 0
else
    status=$?
    ...
    if [[ $status -eq 1 ]]; then
        echo "Test-3 FAIL: The output files differ."
#        echo "Differences:"
#        cat "$diff_file"
        exit 1
    else
