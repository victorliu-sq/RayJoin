...
struct is_nonnarrowing_assignable<T, U, void_t<decltype(T{std::declval<U>()})>> : std::true_type {};
...
class rational {
 public:
  static_assert(std::is_integral<T>::value, "tcb::rational<T> requires T to be an integral type");
  ...
  __device__ __host__ TCB_CONSTEXPR14 rational(value_type num, value_type denom) : num_{num}, denom_{denom} { simplify(); }
  ...
  __device__ __host__ constexpr rational(const rational<U>& other) : num_(other.num()), denom_(other.denom()) {}
  ...
  __device__ __host__ constexpr operator double() const { return static_cast<double>(num_) / static_cast<double>(denom_); }
...
template<typename T>
struct is_rational<rational<T>> : std::conditional_t<std::is_integral<T>::value, std::true_type, std::false_type> {};
...
__device__ __host__ constexpr bool operator==(const T& lhs, const U& rhs) {
  return numerator(lhs) == numerator(rhs) && denominator(lhs) == denominator(rhs);
}
...
__device__ __host__ constexpr auto operator+(const T& lhs, const U& rhs) {
  using value_type = decltype(std::declval<rational_value_t<T>>() + std::declval<rational_value_t<U>>());
  return rational<value_type>{denominator(lhs) * numerator(rhs) + denominator(rhs) * numerator(lhs), denominator(lhs) * denominator(rhs)};
}
...
__device__ __host__ constexpr auto operator-(const T& lhs, const U& rhs) {
  using value_type = decltype(std::declval<rational_value_t<T>>() - std::declval<rational_value_t<U>>());
  return rational<value_type>{numerator(lhs) * denominator(rhs) - numerator(rhs) * denominator(lhs), denominator(lhs) * denominator(rhs)};
}
...
__device__ __host__ constexpr auto operator*(const T& lhs, const U& rhs) {
  using value_type = decltype(std::declval<rational_value_t<T>>() * std::declval<rational_value_t<U>>());
  return rational<value_type>{numerator(lhs) * numerator(rhs), denominator(lhs) * denominator(rhs)};
}
...
__device__ __host__ constexpr auto operator/(const T& lhs, const U& rhs) {
  using value_type = decltype(std::declval<rational_value_t<T>>() / std::declval<rational_value_t<U>>());
  return rational<value_type>{numerator(lhs) * denominator(rhs), denominator(lhs) * numerator(rhs)};
}
...
constexpr auto operator""_rs(unsigned long long i) { return detail::make_literal_proxy<short>(i); }

constexpr auto operator""_rus(unsigned long long i) { return detail::make_literal_proxy<unsigned short>(i); }

constexpr auto operator""_r(unsigned long long i) { return detail::make_literal_proxy<int>(i); }

constexpr auto operator""_ru(unsigned long long i) { return detail::make_literal_proxy<unsigned int>(i); }

constexpr auto operator""_rl(unsigned long long i) { return detail::make_literal_proxy<long>(i); }

constexpr auto operator""_rul(unsigned long long i) { return detail::make_literal_proxy<unsigned long>(i); }

constexpr auto operator""_rll(unsigned long long i) { return detail::make_literal_proxy<long long>(i); }

constexpr auto operator""_rull(unsigned long long i) { return detail::make_literal_proxy<unsigned long long>(i); }

constexpr auto operator""_r8(unsigned long long i) { return detail::make_literal_proxy<std::int_least8_t>(i); }

constexpr auto operator""_ru8(unsigned long long i) { return detail::make_literal_proxy<std::uint_least8_t>(i); }

constexpr auto operator""_r16(unsigned long long i) { return detail::make_literal_proxy<std::int_least16_t>(i); }

constexpr auto operator""_ru16(unsigned long long i) { return detail::make_literal_proxy<std::uint_least16_t>(i); }

constexpr auto operator""_r32(unsigned long long i) { return detail::make_literal_proxy<std::int_least32_t>(i); }

constexpr auto operator""_ru32(unsigned long long i) { return detail::make_literal_proxy<std::uint_least32_t>(i); }

constexpr auto operator""_r64(unsigned long long i) { return detail::make_literal_proxy<std::int_least64_t>(i); }

constexpr auto operator""_ru64(unsigned long long i) { return detail::make_literal_proxy<std::uint_least64_t>(i); }
