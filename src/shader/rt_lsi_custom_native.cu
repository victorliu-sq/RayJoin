...
extern "C" __global__ void __intersection__lsi_custom_native() {
  ...
  for (auto base_eid = begin_eid; base_eid < end_eid; ++base_eid) {
    ...
    if (rayjoin::dev::intersect_test_native<edge_t, edge_t, point_t, coord_t>(
...
extern "C" __global__ void __raygen__lsi_custom_native() {
  ...
  for (unsigned int eid = OPTIX_TID_1D; eid < edges.size(); eid += OPTIX_TOTAL_THREADS_1D) {
    ...
    if (e.b == 0) {
      ...
      x1 = x2 = static_cast<float>(p1.x);
      y1 = static_cast<float>(p1.y);
      y2 = static_cast<float>(p2.y);
    } else {
      if (p1.x > p2.x) SWAP(p1, p2);
      x1 = static_cast<float>(p1.x);
      y1 = static_cast<float>(p1.y);
      x2 = static_cast<float>(p2.x);
      y2 = static_cast<float>(p2.y);
    }

    float3 ray_origin = {x1, y1, 0.0f};
    float3 ray_dir = {x2 - x1, y2 - y1, 0.0f};
