...
class MapOverlayRT : public MapOverlay<CONTEXT_T> {
 ...
 private:
  ...
  void DebugPrintIntersectionsDetailed(int query_map_id, uint32_t max_print = 20) const {
    ...
    auto lsi = std::dynamic_pointer_cast<lsi_t>(this->lsi_);
    if (!lsi) {
      LOG(ERROR) << "DebugPrintIntersectionsDetailed: failed to cast lsi_ to LSIRT";
      return;
    }
    ...
    if (!query_map || !base_map) {
      LOG(ERROR) << "DebugPrintIntersectionsDetailed: null map"
                 << " query_map_id=" << query_map_id << " base_map_id=" << base_map_id;
      return;
    }

    // --------------------------------------------------------------------------
    // Read back GPU counters
    // --------------------------------------------------------------------------
    auto xsectCountVec = readBackStorageBuffer<uint32_t>(lsi->get_xsect_counter_buffer(), 1);
    ...
    const uint32_t n_xsects = xsectCountVec[0];
    LOG(INFO) << "DebugPrintIntersectionsDetailed:"
              << " query_map_id=" << query_map_id << " base_map_id=" << base_map_id << " gpu intersection count=" << n_xsects;

    auto dbg = readBackStorageBuffer<uint32_t>(lsi->get_prof_counter_buffer(), 8);
    if (!dbg.empty()) {
      LOG(INFO) << "LSI DBG:"
                << " raygen=" << (dbg.size() > 0 ? dbg[0] : 0) << " miss=" << (dbg.size() > 1 ? dbg[1] : 0)
                << " inter=" << (dbg.size() > 2 ? dbg[2] : 0) << " writes=" << (dbg.size() > 3 ? dbg[3] : 0)
                << " last_eid=" << (dbg.size() > 4 ? dbg[4] : 0) << " last_prim=" << (dbg.size() > 5 ? dbg[5] : 0)
                << " tested_base_edges=" << (dbg.size() > 6 ? dbg[6] : 0);
    }

    if (n_xsects == 0) {
      LOG(INFO) << "DebugPrintIntersectionsDetailed: no intersections reported";
      return;
    }
    ...
    if (gpuXsects.size() != n_print) {
      LOG(ERROR) << "DebugPrintIntersectionsDetailed: failed to read xsect buffer"
                 << " expected=" << n_print << " actual=" << gpuXsects.size();
      return;
    }
    ...
    auto point_equals = [](const DstPointI64 &a, const DstPointI64 &b) -> bool { return a.x == b.x && a.y == b.y; };

    struct CpuIntersectDebug {
      int64_t e2_p1_agst_e1_raw = 0;
      int64_t e2_p2_agst_e1_raw = 0;
      int64_t e1_p1_agst_e2_raw = 0;
      int64_t e1_p2_agst_e2_raw = 0;

      int64_t e2_p1_agst_e1_final = 0;
      int64_t e2_p2_agst_e1_final = 0;
      int64_t e1_p1_agst_e2_final = 0;
      int64_t e1_p2_agst_e2_final = 0;

      bool e1_same_side_of_e2 = false;
      bool e2_same_side_of_e1 = false;
      bool same_dir = false;
      bool opp_dir = false;
      bool identical_edge = false;
      bool zero_length_reject = false;
      bool result = false;
    };
    ...
    auto cpu_intersect_test_debug = [&](const Edge &e1,
      ...
      d.e2_p1_agst_e1_raw = subedge(e2_p1, e1);
      d.e2_p2_agst_e1_raw = subedge(e2_p2, e1);
      d.e1_p1_agst_e2_raw = subedge(e1_p1, e2);
      d.e1_p2_agst_e2_raw = subedge(e1_p2, e2);

      d.e2_p1_agst_e1_final = d.e2_p1_agst_e1_raw;
      d.e2_p2_agst_e1_final = d.e2_p2_agst_e1_raw;
      d.e1_p1_agst_e2_final = d.e1_p1_agst_e2_raw;
      d.e1_p2_agst_e2_final = d.e1_p2_agst_e2_raw;

      // e1_p1 is on e2
      if (d.e1_p1_agst_e2_final == 0) d.e1_p1_agst_e2_final = -static_cast<int64_t>(e2.a);
      if (d.e1_p1_agst_e2_final == 0) d.e1_p1_agst_e2_final = -static_cast<int64_t>(e2.b);
      if (d.e1_p1_agst_e2_final == 0) {
        d.zero_length_reject = true;
        d.result = false;
        return false;
      }

      if (d.e1_p2_agst_e2_final == 0) d.e1_p2_agst_e2_final = -static_cast<int64_t>(e2.a);
      if (d.e1_p2_agst_e2_final == 0) d.e1_p2_agst_e2_final = -static_cast<int64_t>(e2.b);
      if (d.e1_p2_agst_e2_final == 0) {
        d.zero_length_reject = true;
        d.result = false;
        return false;
      }

      d.e1_same_side_of_e2 = ((d.e1_p1_agst_e2_final > 0 && d.e1_p2_agst_e2_final > 0) || (d.e1_p1_agst_e2_final < 0 && d.e1_p2_agst_e2_final < 0));
      if (d.e1_same_side_of_e2) {
        d.result = false;
        return false;
      }

      // e2_p1 is on e1
      if (d.e2_p1_agst_e1_final == 0) d.e2_p1_agst_e1_final = static_cast<int64_t>(e1.a);
      if (d.e2_p1_agst_e1_final == 0) d.e2_p1_agst_e1_final = static_cast<int64_t>(e1.b);
      if (d.e2_p1_agst_e1_final == 0) {
        d.zero_length_reject = true;
        d.result = false;
        return false;
      }

      if (d.e2_p2_agst_e1_final == 0) d.e2_p2_agst_e1_final = static_cast<int64_t>(e1.a);
      if (d.e2_p2_agst_e1_final == 0) d.e2_p2_agst_e1_final = static_cast<int64_t>(e1.b);
      if (d.e2_p2_agst_e1_final == 0) {
        d.zero_length_reject = true;
        d.result = false;
        return false;
      }

      d.e2_same_side_of_e1 = ((d.e2_p1_agst_e1_final > 0 && d.e2_p2_agst_e1_final > 0) || (d.e2_p1_agst_e1_final < 0 && d.e2_p2_agst_e1_final < 0));
      if (d.e2_same_side_of_e1) {
        d.result = false;
        return false;
      }

      d.same_dir = point_equals(e1_p1, e2_p1) && point_equals(e1_p2, e2_p2);
      d.opp_dir = point_equals(e1_p1, e2_p2) && point_equals(e1_p2, e2_p1);
      d.identical_edge = d.same_dir || d.opp_dir;
    ...
    LOG(INFO) << "DebugPrintIntersectionsDetailed: printing first " << n_print << " GPU-written intersections with full CPU predicate trace";

    for (uint32_t i = 0; i < n_print; ++i) {
      const auto &x = gpuXsects[i];

      // Reconstruct the same role mapping the shader used:
      // if query_map_id == 0:
      //   x.eid0 = query_eid, x.eid1 = base_eid
      // else:
      //   x.eid0 = base_eid, x.eid1 = query_eid
      const uint32_t query_eid = static_cast<uint32_t>((query_map_id == 0) ? x.eid0 : x.eid1);
      const uint32_t base_eid = static_cast<uint32_t>((query_map_id == 0) ? x.eid1 : x.eid0);

      const bool eid_ok = (query_eid < queryEdges.size()) && (base_eid < baseEdges.size());

      if (!eid_ok) {
        LOG(ERROR) << "  [" << i << "] invalid eid mapping"
                   << " query_eid=" << query_eid << " base_eid=" << base_eid << " queryEdges.size=" << queryEdges.size()
                   << " baseEdges.size=" << baseEdges.size() << " raw eid0=" << x.eid0 << " raw eid1=" << x.eid1;
        continue;
      }
      ...
      if (qe.p1_idx >= queryPts.size() || qe.p2_idx >= queryPts.size() || be.p1_idx >= basePts.size() || be.p2_idx >= basePts.size()) {
        LOG(ERROR) << "  [" << i << "] invalid point index"
                   << " query_eid=" << query_eid << " base_eid=" << base_eid << " qe.p1_idx=" << qe.p1_idx << " qe.p2_idx=" << qe.p2_idx
                   << " be.p1_idx=" << be.p1_idx << " be.p2_idx=" << be.p2_idx << " queryPts.size=" << queryPts.size()
                   << " basePts.size=" << basePts.size();
        continue;
      }

      const DstPointI64 &qp1 = queryPts[qe.p1_idx];
      const DstPointI64 &qp2 = queryPts[qe.p2_idx];
      const DstPointI64 &bp1 = basePts[be.p1_idx];
      const DstPointI64 &bp2 = basePts[be.p2_idx];

      CpuIntersectDebug d{};
      const bool cpu_ok = cpu_intersect_test_debug(qe, qp1, qp2, be, bp1, bp2, d);

      const double qx1 = queryScaling.UnscaleX(qp1.x);
      const double qy1 = queryScaling.UnscaleY(qp1.y);
      const double qx2 = queryScaling.UnscaleX(qp2.x);
      const double qy2 = queryScaling.UnscaleY(qp2.y);

      const double bx1 = baseScaling.UnscaleX(bp1.x);
      const double by1 = baseScaling.UnscaleY(bp1.y);
      const double bx2 = baseScaling.UnscaleX(bp2.x);
      const double by2 = baseScaling.UnscaleY(bp2.y);

      LOG(INFO) << "  [" << i << "]"
                << " raw_eid0=" << x.eid0 << " raw_eid1=" << x.eid1 << " | query_eid=" << query_eid << " base_eid=" << base_eid
                << " | cpu_intersect=" << (cpu_ok ? "YES" : "NO") << " | query=(" << qx1 << "," << qy1 << ") -> (" << qx2 << "," << qy2 << ")"
                << " | base=(" << bx1 << "," << by1 << ") -> (" << bx2 << "," << by2 << ")";

      LOG(INFO) << "    query edge coeffs: a=" << qe.a << " b=" << qe.b << " c=" << qe.c << " p1_idx=" << qe.p1_idx << " p2_idx=" << qe.p2_idx
                << " | base edge coeffs: a=" << be.a << " b=" << be.b << " c=" << be.c << " p1_idx=" << be.p1_idx << " p2_idx=" << be.p2_idx;

      LOG(INFO) << "    raw subedge:"
                << " e2_p1_agst_e1=" << d.e2_p1_agst_e1_raw << " e2_p2_agst_e1=" << d.e2_p2_agst_e1_raw << " e1_p1_agst_e2=" << d.e1_p1_agst_e2_raw
                << " e1_p2_agst_e2=" << d.e1_p2_agst_e2_raw;

      LOG(INFO) << "    final subedge after tie-break:"
                << " e2_p1_agst_e1=" << d.e2_p1_agst_e1_final << " e2_p2_agst_e1=" << d.e2_p2_agst_e1_final
                << " e1_p1_agst_e2=" << d.e1_p1_agst_e2_final << " e1_p2_agst_e2=" << d.e1_p2_agst_e2_final;

      LOG(INFO) << "    decisions:"
                << " e1_same_side_of_e2=" << (d.e1_same_side_of_e2 ? "YES" : "NO") << " e2_same_side_of_e1=" << (d.e2_same_side_of_e1 ? "YES" : "NO")
                << " same_dir=" << (d.same_dir ? "YES" : "NO") << " opp_dir=" << (d.opp_dir ? "YES" : "NO")
                << " identical_edge=" << (d.identical_edge ? "YES" : "NO") << " zero_length_reject=" << (d.zero_length_reject ? "YES" : "NO")
                << " final=" << (d.result ? "YES" : "NO");

      if (!cpu_ok) {
        LOG(ERROR) << "    GPU wrote this pair, but CPU predicate rejected it"
                   << " query_eid=" << query_eid << " base_eid=" << base_eid << " raw_eid0=" << x.eid0 << " raw_eid1=" << x.eid1;
      }
    }
  }
};
