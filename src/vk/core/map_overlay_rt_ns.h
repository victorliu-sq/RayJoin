...
class MapOverlayRTNS : public MapOverlayNS<CONTEXT_NS_T> {
 ...
 public:
  ...
  // void ComputeOutputPolygonsHost() {
  ...
  //         accel_[base_map_id].GetTraverseHandle(),
  //         LaunchParamsPIP{
  //             .query_map_id = static_cast<int32_t>(query_map_id), .query_point_count = static_cast<uint32_t>(tasks.size()), ._pad0 = 0u, ._pad1 =
  //             0u},
  ...
  void ComputeOutputPolygonsHost() {
    ...
    for (int im = 0; im < 2; ++im) {
      ...
      for (size_t group_idx = 0; group_idx < unique_eids.size(); ++group_idx) {
        ...
        for (size_t i = begin; i < end; ++i) {
          ...
          refs.push_back(LocalRef{
              i,
              dist2_from_edge_start(x),
              base_eid_of(x),
              static_cast<long double>(x.x),
              static_cast<long double>(x.y),
          });
        }
        ...
        for (size_t local_idx = 0; local_idx + 1 < n_xsect; ++local_idx) {
          ...
          point_t mp{};
          mp.x = x1.x + (x2.x - x1.x) * coord_t(0.5);
          mp.y = x1.y + (x2.y - x1.y) * coord_t(0.5);
      ...
      VkDeviceBuf mid_point_in_polygon_buf;
      mid_point_in_polygon_buf.Init(sizeof(polygon_id_t) * tasks.size());
      ...
      RunComputePass(static_cast<uint32_t>(tasks.size()),
                     finalize_spv.c_str(),
                     LaunchParamsPIPFinalize{.point_count = static_cast<uint32_t>(tasks.size()),
                                             .exterior_face_id = static_cast<int32_t>(EXTERIOR_FACE_ID),
                                             ._pad0 = 0u,
                                             ._pad1 = 0u},
                     base_map->getEdgesBuffer(),
                     base_map->getPointsBuffer(),
  ...
  //     static_assert(sizeof(XsectSortEntry) == 16);
  //
  ...
  //
  //       RunComputePass(sort_count,
  //                      prepare_spv.c_str(),
  //                      params,
  //                      xsect_buf_,  // binding 0 -> gXsectsIn
  //                      xsect_edges_sorted_buf,  // binding 1 -> gXsectsOut
  //                      xsect_sort_entries_buf);  // binding 2 -> gSortEntries
  //     }
  //
  ...
  //
  //     if (tasks.empty()) {
  //       // Persist the sorted + reset-midpoint version too.
  //       xsect_edges_sorted_member = xsect_edges_sorted;
  //       continue;
  //     }
  //
  ...
  //     }
  //
  //     // Persist final host result so DumpComputeOutputPolygonsCSV / WriteResult see it.
  //     xsect_edges_sorted_member = xsect_edges_sorted;
  //   }
  //
  //   if (rayjoin::ShouldDumpStage(config_.dump_results, "pipmid")) {
